...
core_freq_t FrequencyActuatorCommon::closestFreq(int freqMHz)
{
...
void FrequencyActuatorCommon::_identify_sys(const sys_info_t & sys_info)
{
	for(int domain_id = 0; domain_id < sys_info.freq_domain_list_size; ++domain_id){
		...
		_freq_max[domain_id] = SIZE_COREFREQ;
		for(int f = 0; f < SIZE_COREFREQ; ++f){
			if(pal_arch_has_freq(arch,(core_freq_t)f)){
				_freq_max[domain_id] = (core_freq_t)f;
				break;
			}
		}
		if(_freq_max[domain_id] == -1) arm_throw(DaemonSystemException,"Cannot find maximum frequency");

		_freq_min[domain_id] = SIZE_COREFREQ;
		for(int f = SIZE_COREFREQ-1; f >=0; --f){
			if(pal_arch_has_freq(arch,(core_freq_t)f)){
				_freq_min[domain_id] = (core_freq_t)f;
				break;
			}
		}
		if(_freq_min[domain_id] == -1) arm_throw(DaemonSystemException,"Cannot find minimum frequency");
	}
}
