...
##################################################################
# Parses all traces saved at $TRACE_OUTPUT_DIR-$RTS_ARCH-$RTS_ARCH
# (as defined in the confs.sh)
#
...
RAWS=$(find  $TGT_TRACES_DIR -type d -name "*.raw" -print0 | xargs -0)
for raw in $RAWS
do
    ...
    then
        ...
        sh $SPARTA_SCRIPTDIR/tracing/trace_parse.sh $raw
        echo -e "\n"
    else
